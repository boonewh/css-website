        <main>
            <div class="therapy-container2">
                <h1>Teen and Adolescent Therapy</h1>
                <br>
            </div>
            <br>
            <div class= "therapy">
                <section>
                    <img src="assets/images/teen.jpg" alt="Teen and Adolescent Therapy"> 
                    <h2>Transform Your Teen and Family Dynamics</h2>
                    <p>The teenage years bring their own unique set of challenges! Teens are no longer children, but not yet adults. Adolescence can be a difficult time, especially in our society today, where social media and peer pressure compound this already difficult stage of life.</p>
                    
                    <p>Our therapists at The Center for Social Success are attuned to the unique plight of adolescence and are masterful at connecting with teens and helping them to come up with healthy coping skills. Our team of friendly and professional therapists is dedicated to creating a safe space where teens can freely express themselves, fostering resilience and lasting transformations. Grounded in cognitive-behavioral techniques, our therapists engage with teens and adolescents not only through traditional talk-based therapy, but can also include such therapies as sand tray therapy, art therapy, EMDR, and group therapy.</p>
                    
                    <p>While being a teenager is difficult, parenting a teenager can be just as difficult. At The Center for Social Success, we go beyond individual teen and adolescent sessions by offering parent or family sessions. These collaborative sessions provide parents with valuable insights on how to best support and parent their teens. Whether in a traditional talk therapy format or within our specialized playroom, sand tray, or art room, these sessions aim to enhance family connections and understanding. Experience a holistic approach to healing and growth for both teens and their families.</p>
                    
                    <h3>Some common areas of focus include:</h3>
                    <ul>
                        <li>Identifying and Expressing Emotions</li>
                        <li>Anger Management</li>
                        <li>Anxiety Management</li>
                        <li>Identity Development</li>
                        <li>ADHD</li>
                        <li>Autism</li>
                        <li>Conflict resolution</li>
                        <li>Social and Peer Relationships</li>
                        <li>Self-Esteem</li>
                        <li>Self-Regulation</li>
                        <li>Social Anxiety</li>
                        <li>Executive Functioning</li>
                        <li>Coping with Divorce</li>
                        <li>College and College Readiness</li>
                        <li>Coping Strategies and Problem Solving</li>
                        <li>Trauma</li>
                        <li>And much more!</li>
                    </ul>
                    
                    <p><strong>Transform your teen and family dynamics with our positive and empowering therapy services. Call today to discover practical strategies designed to address longstanding issues and witness positive changes unfold!</strong></p>

                </section>   
            </div>     
        </main>